...
import { initMeetings } from "../data/meetings";
...
export function MeetingCalendar() {
  ...
  const handleSlotClick = useCallback(
    ...
    [confirmMeetingCreation, meetingsByDay, weekMinDate, weekMaxDate, authUser],
  );
  ...
  const handleResize = useCallback(
    (
      ...
      setMeetings((prev) => {
        ...
        if (meeting.ownerId !== currentUserId) return prev;
        ...
        return prev.map((m) => (m.id === meetingId ? { ...m, start, end } : m));
      });
    },
    [currentUserId],
  );
