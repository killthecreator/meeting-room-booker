    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-secondary-400 text-sm font-light tracking-wide">Loading...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-white/60 bg-white/70 px-6 py-3 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          <span className="text-secondary-500 text-sm font-medium">{user.name}</span>
          <button
            onClick={logout}
            className="cursor-pointer rounded-lg border border-secondary-200 bg-white px-3 py-1.5 text-xs font-medium text-secondary-600 shadow-sm transition-all duration-200 hover:border-secondary-300 hover:text-secondary-800 hover:shadow active:scale-[0.97]"
          >
            Sign out
          </button>
        </div>
